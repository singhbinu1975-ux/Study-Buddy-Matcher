...
import { proposeSession, confirmSession, cancelSession } from "@/app/actions/session";
...
export default function ChatBox({
  ...
  }, [matchId, isPusherConfigured, pusherKey, pusherCluster]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await sendMessage(matchId, textToSend);
  ...
  const handleProposeSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await proposeSession(matchId, isoString);
      if (res.success && res.session) {
        setSession({
          id: res.session.id,
          matchId: res.session.matchId,
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-950/40 text-left relative">
        ...
        session.proposerId === currentUserId ? (
          <div className="px-5 py-3.5 bg-indigo-500/10 border-b border-indigo-500/25 flex flex-col sm:flex-row items-center justify-between text-xs gap-3">
            <div className="text-slate-300">
              📅 You proposed a study session for <strong>{new Date(session.scheduledTime).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</strong>. Waiting for {partnerName} to confirm...
            </div>
            <button
          ...
          <div className="px-5 py-3.5 bg-amber-500/10 border-b border-amber-500/25 flex flex-col sm:flex-row items-center justify-between text-xs gap-3">
            <div className="text-slate-300">
              📅 {partnerName} proposed a study session for <strong>{new Date(session.scheduledTime).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</strong>.
            </div>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  const dateStr = session.scheduledTime.split("T")[0];
                  setInputDate(dateStr);
                  setShowScheduler(true);
                }}
                className="px-3.5 py-1.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-slate-200 font-semibold transition-colors"
              >
                Propose Alternative
              </button>
            </div>
      ...
      {session && session.status === "confirmed" && (
        <div className="px-5 py-3.5 bg-emerald-500/10 border-b border-emerald-500/25 flex flex-col sm:flex-row items-center justify-between text-xs gap-3">
          <div className="text-slate-300">
            🎉 <strong>Upcoming Session Confirmed:</strong> scheduled on <strong>{new Date(session.scheduledTime).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</strong>!
          </div>
          <button
            onClick={handleCancelSession}
            className="px-3 py-1.5 rounded-lg border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/25 text-rose-300 transition-colors font-semibold"
        ...
        <div className="absolute inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            <div className="space-y-1">
              <h4 className="text-base font-bold text-white">Propose Study Session</h4>
              <p className="text-[11px] text-slate-400">Select a date and time to meet with {partnerName}.</p>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[300px]">
        {messages.length > 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center py-12 space-y-1">
            ...
            <p className="text-slate-400 text-xs">Be the first to say hello and plan your study dates!</p>
          </div>
        )}
